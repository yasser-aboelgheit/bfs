...
class TestRegisterNodeViewSet(APITestCase):
    ...
    def test_nodes_connected(self):
        ...
        self.assertFalse(Edge.objects.filter(
            node_from=node_a, node_to=node_b).exists())
    ...
    def test_empty_field(self):
        ...
        self.assertEqual(response.json(), {
                         'node_to': ['This field is required.']})

        self.data = {"node_to": "B", }
        response = self.client.post(self.url, self.data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json(), {
                         'node_from': ['This field is required.']})
...
class TestDisplayNodeViewSet(APITestCase):
    ...
    def test_get_path(self):
        response = self.client.get(
            self.url, {'node_from': self.node1.name, "node_to": self.node2.name})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data.get("path"), "%s, %s" %
                         (self.node1.name, self.node2.name))

    def test_no_path(self):
        response = self.client.get(
            self.url, {'node_from': self.node1.name, "node_to": self.node3.name})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
